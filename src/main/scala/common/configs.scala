...
class DefaultBoomConfig extends Config((site, here, up) => {
   ...
   case BoomTilesKey => up(BoomTilesKey, site) map { r => r.copy(
      ...
      icache = Some(ICacheParams(fetchBytes = 4*4, rowBits = site(SystemBusKey).beatBits, nSets=128, nWays=4))
      )}
...
class WithSmallBooms extends Config((site, here, up) => {
   case BoomTilesKey => up(BoomTilesKey, site) map { r =>r.copy(
      core = r.core.copy(
         ...
         numLsuEntries = 8,
         maxBrCount = 4,
         btb = BoomBTBParameters(nSets=64, nWays=4, nRAS=8, tagSz=20),
...
class WithMediumBooms extends Config((site, here, up) => {
   case BoomTilesKey => up(BoomTilesKey, site) map { r =>r.copy(
      core = r.core.copy(
         ...
         regreadLatency = 1,
         renameLatency = 2,
         btb = BoomBTBParameters(nSets=64, nWays=4, nRAS=8, tagSz=20, bypassCalls=false, rasCheckForEmpty=false),
