...
object VecInstructions {
  def VADD               = BitPat("b1000000??????????1???????1100111")
  def VSUB               = BitPat("b1000001??????????1???????1100111")
  def VMUL               = BitPat("b1000110??????????1???????1100111")
   // TODO_VEC: Support predicated madd operations
  def VMADD              = BitPat("b?????00??????????011?????1100111")
  def VMSUB              = BitPat("b?????01??????????011?????1100111")
  def VNMADD             = BitPat("b?????10??????????011?????1100111")
  def VNMSUB             = BitPat("b?????11??????????011?????1100111")
  def VLD                = BitPat("b?????0000000?????1???????0000111")
  def VLDS               = BitPat("b?????01??????????1???????0000111")
  def VLDX               = BitPat("b?????10??????????1???????0000111")
  def VST                = BitPat("b?????0000000?????1???????0100111")
  def VSTS               = BitPat("b?????01??????????1???????0100111")
  def VSTX               = BitPat("b?????10??????????1???????0100111")
  def VINSERT            = BitPat("b1011100??????????1???????1100111")
  def VEXTRACT           = BitPat("b1010000??????????1???????1100111")
  def VADDI              = BitPat("b0000?????????????1???????1100111")
}
...
object VecCSRs {
  ...
  val all = Seq(vcs, vl, vxrm, vxsat, vmaxew,
      vcfg0, vcfg1, vcfg2, vcfg3,
      vcfg4, vcfg5, vcfg6, vcfg7,
      vcfg8, vcfg9, vcfg10, vcfg11,
      vcfg12, vcfg13, vcfg14, vcfg15)
}
}
