...
//   val decodeWidth     = boomParams.decodeWidth
   ...
   val ftqSz            = boomParams.ftq.nEntries      // number of FTQ entries should match (or slightly exceed) ROB rows, unless we start filtering out speculatively safe fetch bundles
