   ...
   with HasBoomCoreIO
{
  ...
  if (usingFPU) {
    for (w <- 0 until coreWidth) {
      ...
      dis_uops(w).prs1_busy := (dis_uops(w).lrs1_rtype === RT_FLT && f_uop.prs1_busy) ||
                               (dis_uops(w).lrs1_rtype === RT_FIX && i_uop.prs1_busy)
      dis_uops(w).prs2_busy := (dis_uops(w).lrs2_rtype === RT_FLT && f_uop.prs2_busy) || 
                               (dis_uops(w).lrs2_rtype === RT_FIX && i_uop.prs2_busy)
      dis_uops(w).prs3_busy := dis_uops(w).frs3_en && f_uop.prs3_busy
