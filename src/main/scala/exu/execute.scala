...
import chisel3._
import chisel3.util.{ValidIO, DecoupledIO, PriorityMux, PopCount}
...
{
   val uop = new MicroOp()
   val flags = UInt(tile.FPConstants.FLAGS_SZ.W)
}
   ...
   )(implicit p: Parameters) extends BoomBundle()(p)
{
   ...
   val fu_types = Output(UInt(FUC_SZ.W))
      ...
      has_ifpu = has_ifpu)(p)
{
   ...
   fpu_resp_fflags.bits.uop := DontCare
   fpu_resp_fflags.bits.flags := DontCare
   ...
   val fdiv_resp_data = Wire(UInt(65.W))
   fdiv_resp_data := DontCare
   val fdiv_resp_fflags = Wire(new ValidIO(new FFlagsResp()))
   fdiv_resp_fflags.bits.uop := DontCare
   fdiv_resp_fflags.bits.flags := DontCare
