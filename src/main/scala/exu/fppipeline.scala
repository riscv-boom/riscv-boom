...
class FpPipeline(implicit p: Parameters) extends BoomModule()(p) with tile.HasFPUParameters
{
   ...
   val io = new Bundle
   {
      ...
      val tovec            = Decoupled(new ExeUnitResp(xLen))           // to Vector issue
   ...
   for (w <- 0 until DISPATCH_WIDTH)
   {
      ...
      when (usingVec.B && io.dis_uops(w).vec_val && (
         ...
         issue_unit.io.dis_uops(w).prs3_busy := io.dis_uops(w).prs3_busy && io.dis_uops(w).lrs3_rtype === RT_FLT
      }
   ...
   for ((writeback, issue_wakeup) <- io.wakeups zip issue_unit.io.wakeup_pdsts)
   {
      ...
      issue_wakeup.bits.eidx     := 0.U
      issue_wakeup.bits.poisoned := false.B
      issue_wakeup.bits.vp_pdst  := 0.U
      issue_wakeup.bits.writes_vp := false.B
   }
