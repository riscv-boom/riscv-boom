...
import chisel3._
import chisel3.util.{DecoupledIO, MuxLookup, Cat, Valid, BitPat, log2Ceil, ValidIO, Pipe}
...
class BrResolutionInfo(implicit p: Parameters) extends BoomBundle()(p)
{
   ...
   def getCfiIdx = pc_lob >> log2Ceil(minCoreInstBytes)
...
class ALUUnit(is_branch_unit: Boolean = false, num_stages: Int = 1)(implicit p: Parameters)
             extends PipelinedFunctionalUnit(num_stages = num_stages
                                            ...
                                            , data_width = 64  //xLen
                                            , is_branch_unit = is_branch_unit)(p) with HasL1ICacheBankedParameters
{
   ...
   if (is_branch_unit)
   {
      val curr_pc = (AlignPCToBoundary(io.get_ftq_pc.fetch_pc, icBlockBytes).asSInt + Mux( uop.acrossb, -2.S, Cat( 0.U, io.req.bits.uop.pc_lob).asSInt)).asUInt
   ...
   val op2_data = Mux(io.req.bits.uop.ctrl.op2_sel === OP2_IMM,  Sext(imm_xprlen.asUInt, xLen),
                  ...
                  Mux(io.req.bits.uop.ctrl.op2_sel === OP2_FOUR, Mux( uop.rvc, 2.U, 4.U),
                                                                 0.U))))
   ...
   if (is_branch_unit)
   {
      val uop_pc_ = (AlignPCToBoundary(io.get_ftq_pc.fetch_pc, icBlockBytes).asSInt + Mux( uop.acrossb, -2.S, Cat( 0.U, io.req.bits.uop.pc_lob).asSInt)).asUInt
      ...
      val pc_plus4 = (uop_pc_ + Mux( uop.rvc, 2.U, 4.U))(vaddrBits,0)
      ...
      when (is_br_or_jalr && pc_sel === PC_BRJMP && !mispredict && io.get_ftq_pc.next_val)
      {
         ...
         when (io.get_ftq_pc.next_pc(vaddrBits, log2Ceil(minCoreInstBytes)) =/= bj_addr(vaddrBits, log2Ceil(minCoreInstBytes)))
         {
            printf ("[FuncUnit] Branch jumped to 0x%x, should have jumped to 0x%x.\n",
               io.get_ftq_pc.next_pc, bj_addr)
         }
         /*assert (io.get_ftq_pc.next_pc(vaddrBits, log2Ceil(minCoreInstBytes)) === bj_addr(vaddrBits, log2Ceil(minCoreInstBytes)),
      ...
      val lowpc = uop.pc_lob
      brinfo.pc_lob         := Mux( lowpc(log2Ceil(rvcFetchWidth),1) === (rvcFetchWidth-1).U && !uop.rvc, alignToFetchBoundary(io.get_ftq_pc.fetch_pc), lowpc)
      ...
      br_unit.btb_update.bits.pc               := alignToFetchBoundary(io.get_ftq_pc.fetch_pc)// tell the BTB which pc to tag check against
      br_unit.btb_update.bits.cfi_pc           := Mux( lowpc(log2Ceil(rvcFetchWidth),1) === (rvcFetchWidth-1).U && !uop.rvc, alignToFetchBoundary(io.get_ftq_pc.fetch_pc), lowpc)
      br_unit.btb_update.bits.target           := (target.asSInt & (-minCoreInstBytes).S).asUInt
   ...
   data_width = 65)(p)
{
   ...
   io.req.ready := true.B // TODO "verify"
