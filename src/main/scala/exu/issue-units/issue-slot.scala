...
class IssueSlotIO(val numWakeupPorts: Int)(implicit p: Parameters) extends BoomBundle
{
  ...
  val slot_uop      = Output(new MicroOp())
  ...
  extends BoomModule
{
  ...
  val next_valid = WireInit(!IsKilledByBranch(io.brupdate, slot_uop) && slot_valid)
  val next_uop   = WireInit(UpdateBrMask(io.brupdate, slot_uop))

  io.valid         := slot_valid
  io.slot_uop      := slot_uop
  ...
  when (io.grant && !squash_grant) {
    assert (!slot_uop.prs1_busy && !slot_uop.prs2_busy && !slot_uop.prs3_busy)
    next_valid := false.B
    io.iss_uop.valid := !io.kill
  }
  ...
  for (wakeup <- io.wakeup_ports) {
    when (wakeup.valid) {
      when (wakeup.bits.pdst === slot_uop.prs1) { next_uop.prs1_busy := false.B }
      when (wakeup.bits.pdst === slot_uop.prs2) { next_uop.prs2_busy := false.B }
      when (wakeup.bits.pdst === slot_uop.prs3) { next_uop.prs3_busy := false.B }
    }
  }
  ...
  io.request := slot_valid && !slot_uop.prs1_busy && !slot_uop.prs2_busy && !slot_uop.prs3_busy && !slot_uop.ppred_busy
}
