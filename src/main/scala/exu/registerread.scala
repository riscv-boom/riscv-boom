...
import chisel3._
import chisel3.util.{DecoupledIO, MuxCase}
...
)(implicit p: Parameters) extends BoomModule()(p)
{
   ...
   val exe_reg_rs1_data = Reg(Vec(issue_width, UInt(register_width.W)))
   val exe_reg_rs2_data = Reg(Vec(issue_width, UInt(register_width.W)))
   val exe_reg_rs3_data = Reg(Vec(issue_width, UInt(register_width.W)))
   ...
   val rrd_rs1_data   = Wire(Vec(issue_width, UInt(regwidth.W)))
   val rrd_rs2_data   = Wire(Vec(issue_width, UInt(regwidth.W)))
   val rrd_rs3_data   = Wire(Vec(issue_width, UInt(regwidth.W)))
   ...
   val bypassed_rs1_data = Wire(Vec(issue_width, UInt(register_width.W)))
   val bypassed_rs2_data = Wire(Vec(issue_width, UInt(register_width.W)))
