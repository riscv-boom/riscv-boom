   ...
   (implicit p: Parameters) extends BoomBundle()(p)
{
   ...
   val debug = Output(new Bundle { val busytable= UInt(num_pregs.W) })
}
   ...
   (implicit p: Parameters) extends BoomModule()(p)
{
   ...
   val prs1_was_bypassed = VecInit(Seq.fill(pl_width) {WireInit(false.B)})
   val prs2_was_bypassed = VecInit(Seq.fill(pl_width) {WireInit(false.B)})
   val prs3_was_bypassed = VecInit(Seq.fill(pl_width) {WireInit(false.B)})
   for {
      w <- 0 until pl_width
      xx <- w-1 to 0 by -1
   }{
