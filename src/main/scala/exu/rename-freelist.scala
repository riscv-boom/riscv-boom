...
{
   val freelist = UInt(num_phys_registers.W)
   val isprlist = UInt(num_phys_registers.W)
}
   ...
   (implicit p: Parameters) extends BoomModule()(p)
{
   ...
   val allocation_lists = Reg(Vec(MAX_BR_COUNT, UInt(num_phys_registers.W)))

   // TODO why is this a Vec? can I do this all on one bit-vector?
   val enq_mask = Wire(Vec(pl_width, UInt(num_phys_registers.W)))
   ...
   val req_free_list = Wire(UInt(num_phys_registers.W))
   val enq_free_list = Wire(UInt(num_phys_registers.W))
   ...
   val allocation_list = Wire(UInt(num_phys_registers.W))
   ...
   if (ENABLE_COMMIT_MAP_TABLE)
   {
      ...
      val com_mask = Wire(Vec(pl_width, UInt(num_phys_registers.W)))
      val stale_mask = Wire(Vec(pl_width, UInt(num_phys_registers.W)))
      for (w <- 0 until pl_width)
      {
