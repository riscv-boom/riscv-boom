...
import chisel3._
import chisel3.util.{log2Ceil, Valid, RegEnable}
   ...
   (implicit p: Parameters) extends BoomBundle()(p)
{
   ...
   val ren1_mask = Output(Vec(pl_width, Bool())) // mask of valid instructions
   val ren1_uops = Output(Vec(pl_width, new MicroOp()))

   // physical specifiers available AND busy/ready status available.
   val ren2_mask  = Output(Vec(pl_width, Bool())) // mask of valid instructions
   val ren2_uops  = Output(Vec(pl_width, new MicroOp()))
...
{
   val ifreelist = UInt(int_num_pregs.W)
   val iisprlist = UInt(int_num_pregs.W)
   val ibusytable = UInt(int_num_pregs.W)
   val ffreelist = UInt(fp_num_pregs.W)
   val fisprlist = UInt(fp_num_pregs.W)
