  ...
  )(implicit p: Parameters) extends BoomModule
{
  ...
  val s_reset :: s_normal :: s_wait_till_empty :: Nil = Enum(3)
...
//                  "FP instruction writing back exc bits is overriding an existing exception.")
  ...
  val is_mini_exception = io.com_xcpt.bits.cause === MINI_EXCEPTION_MEM_ORDERING || io.com_xcpt.bits.cause === MINI_EXCEPTION_STORE_BLOCKED
  ...
  if (enableFastPNR) {
    ...
    when (RegNext(RegNext(flush_val))) { pnr_maybe_at_tail := false.B }
  }
  ...
  when (RegNext(RegNext(flush_val))) {
    rob_tail     := 0.U
    rob_tail_lsb := 0.U
    rob_head     := 0.U
    rob_head_lsb := 0.U
