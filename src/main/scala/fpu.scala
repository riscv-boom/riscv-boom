...
//      val dpath = (new DpathFPUIO).flip
   ...
   fpiu.io.in.bits := req
   val fpiu_out = Pipe(Reg(next=fpiu.io.in.valid), fpiu.io.out.bits.toint, fpu_latency-1)
                         
 
   val fpmu = Module(new rocket.FPToFP(3)) // 2 for rocket
   ...
   io.resp.valid := ifpu.io.out.valid || 
                    fpiu_out.valid ||
                    fpmu.io.out.valid
   io.resp.bits.data := Mux(ifpu.io.out.valid, ifpu.io.out.bits.data,
                        Mux(fpiu_out.valid,    fpiu_out.bits,
                                               fpmu.io.out.bits.data))
}
