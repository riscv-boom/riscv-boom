...
class FrontendResp extends BOOMCoreBundle {
  ...
  val data = Bits(width = fetchWidth*4*8) //TODO Add ibytes in there
...
class Frontend extends Module with BOOMCoreParameters
{
  ...
  val btb = Module(new BTB(fetchWidth))
  ...
  require (fetchWidth == 1 || fetchWidth == 2)
  val msb = vaddrBits-1
  ...
  val pcp4_0 = s1_pc + UInt(fetchWidth * 4)
