 ...
 with HasBoomFrontendParameters
{
  ...
  val banked_predictors = Seq.fill(nBanks) { Module(new NullBranchPredictorBank) }
  val banked_lhist_providers = Seq.fill(nBanks) { Module(new NullLocalBranchPredictorBank) }
