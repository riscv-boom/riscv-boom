...
import chisel3._
import chisel3.util.{Valid, log2Ceil, PriorityEncoder}
   ...
   with HasL1ICacheBankedParameters
{
   val io = IO(new Bundle
   {
      ...
      val rvc_mask      = Input(UInt(fetch_width.W))
      val fidx          = Input(Vec(fetch_width, UInt(log2Ceil(2*fetch_width).W)))
      ...
      val btb_idx       = Input(UInt(log2Ceil(fetch_width).W))
      val split         = Input(Bool())
      val btb_incurr    = Input(Bool())
      val bpd_taken     = Input(Bool())
   ...
   val btb_idx = io.btb_idx
   val btb_target = io.btb_resp.bits.target // TODO uncomment .sextTo(vaddrBitsExtended)
   val bpd_predicted_taken = io.bpd_taken

   when (io.btb_resp.valid && io.btb_incurr)
   {
   ...
   val btb_was_wrong = io.btb_resp.valid && (wrong_cfi || wrong_target || (!io.inst_mask(btb_idx) && io.btb_incurr))
   ...
   val jal_wins = io.is_jal.reduce(_|_) &&
      (!btb_hit ||
      btb_was_wrong ||
      (jal_idx < btb_idx) || (btb_hit && io.split && !io.btb_incurr) ||
   ...
   io.btb_update.bits.pc := io.aligned_pc
   io.btb_update.bits.target := io.jal_targs(jal_idx)
   io.btb_update.bits.taken := true.B
   io.btb_update.bits.cfi_pc := io.fidx(jal_idx) << log2Ceil(minCoreInstBytes)
   ...
   val ret_wins = io.is_ret(ret_idx) && btb_hit && io.btb_incurr && (ret_idx === btb_idx)
   val jal_may_win = io.is_jal.reduce(_|_) && (!btb_hit || btb_was_wrong || jal_idx < btb_idx || (btb_hit && io.split && !io.btb_incurr) || (btb_hit && io.btb_incurr && (btb_idx === jal_idx)))
   io.ras_update.valid := jal_may_win && io.is_call(jal_idx) || ret_wins
   io.ras_update.bits.is_call := io.is_call(jal_idx)
   io.ras_update.bits.is_ret := io.is_ret(ret_idx)
   io.ras_update.bits.return_addr := io.aligned_pc + (io.fidx(jal_idx) << log2Ceil(minCoreInstBytes).U) + Mux( io.rvc_mask(jal_idx), 2.U, 4.U)
}
