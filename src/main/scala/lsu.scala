...
//   val laq_block_id     = Vec.fill(num_ld_entries) { Reg() { UInt(width = MEM_ADDR_SZ) } }  // TODO something is blocking us from executing, listen for this ID to wakeup
   ...
   val pf_ld = dtlb.io.req.valid && dtlb.io.resp.xcpt_ld && isRead(exe_tlb_uop.mem_cmd)
   val pf_st = dtlb.io.req.valid && dtlb.io.resp.xcpt_st && isWrite(exe_tlb_uop.mem_cmd)
