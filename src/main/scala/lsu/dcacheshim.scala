...
import chisel3._
import chisel3.util.{DecoupledIO, ValidIO, log2Ceil, isPow2, Fill}
...
{
   val data         = UInt(coreDataBits.W)
   val data_subword = UInt(coreDataBits.W)
   val uop          = new MicroOp
   val typ          = UInt(freechips.rocketchip.rocket.MT_SZ.W)
}
   ...
   with freechips.rocketchip.rocket.constants.MemoryOpConstants
{
   ...
   val m1_inflight_tag  = Wire(UInt()) // one cycle ago, aka now in the Mem1 Stage
   val m2_inflight_tag  = Wire(UInt()) // two cycles ago, aka now in the Mem2 Stage
   ...
   io.core.nack.valid     := (io.dmem.s2_nack) || RegNext(io.core.req.bits.kill) || RegNext(iflb_kill) ||
                              RegNext(RegNext(io.core.req.valid && !(io.dmem.req.ready), false.B))
