...
import chisel3._
import chisel3.util.{ValidIO, Valid, Cat, Fill, isPow2, PriorityEncoder, RegEnable, PopCount, log2Ceil, MuxCase, DecoupledIO}
...
class LoadStoreUnitIO(val pl_width: Int)(implicit p: Parameters) extends BoomBundle()(p)
{
   ...
   // Execute Stage
   val exe_resp           = Flipped(new DecoupledIO(new FuncUnitResp(xLen)))
...
//   val laq_block_id     = Vec.fill(num_ld_entries) { Reg() { UInt(width = MEM_ADDR_SZ) } }  // TODO something is blocking us from executing, listen for this ID to wakeup
   ...
   val mem_tlb_miss     = RegNext(tlb_miss, false.B)
   val mem_tlb_uncacheable = RegNext(tlb_addr_uncacheable, false.B)
   ...
   val mem_fired_sta = RegNext(will_fire_sta_incoming || will_fire_sta_retry, false.B)
   val mem_fired_stdi = RegNext(will_fire_std_incoming, false.B)
   val mem_fired_stdf = RegNext(io.fp_stdata.valid, false.B)
   val mem_fired_sfence = RegNext(will_fire_sfence, false.B)

   mem_ld_killed := false.B
   when (RegNext((IsKilledByBranch(io.brinfo, exe_ld_uop) ||
         io.exception ||
...
//               (mem_ld_uop.mem_typ =/= stq_uop(i).mem_typ) &&
   ...
   val do_ldld_search = RegNext(will_fire_load_incoming ||
                                   will_fire_load_retry ||
                                   will_fire_load_wakeup,
                            false.B) && MCM_ORDER_DEPENDENT_LOADS.B
...
//   ++ failed_loads)).asUInt)
   val temp_bits = (VecInit(Seq.tabulate(num_ld_entries)(i =>
