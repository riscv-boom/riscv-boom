...
class TageAllocateEntryInfo(index_sz: Int, tag_sz: Int) extends Bundle //TageIndex(index_sz)
{
   val index = UInt(width = index_sz)
   val tag = Bits(width=tag_sz)
   val taken = Bool()
   override def cloneType: this.type = new TageAllocateEntryInfo(index_sz, tag_sz).asInstanceOf[this.type]
}
   ...
   )(implicit p: Parameters) extends BoomBundle()(p)
{
   ...
   val allocate = (new ValidIO(new TageAllocateEntryInfo(index_sz, tag_sz))).flip
   def AllocateNewEntry(idx: UInt, tag: UInt, taken: Bool) =
   {
   ...
   )(implicit p: Parameters) extends BoomModule()(p)
{
   ...
   val ubit_table    = Mem(num_entries, Bits(width = 1))
...
//      (addr >> UInt(4))(tag_sz-1,0)
   ...
   when (io.allocate.valid)
   {
      ...
      ubit_table(a_idx)    := Bits(0)
      tag_table(a_idx)     := io.allocate.bits.tag
      counter_table(a_idx) := Mux(io.allocate.bits.taken, UInt(CNTR_WEAK_TAKEN), UInt(0))
   ...
   if (DEBUG_PRINTF_TAGE)
   {
      ...
      {
         printf("   (%d) [tag=0x%x] [c=%d] [u=%d]"
            , UInt(i,4), tag_table(UInt(i)), counter_table(UInt(i)), ubit_table(UInt(i)))
         printf("(%d) [tag=0x%x] [c=%d] [u=%d]"
            , UInt(i+1,4), tag_table(UInt(i+1)) & UInt(0xffff), counter_table(UInt(i+1)), ubit_table(UInt(i+1)))
         printf("(%d) [tag=0x%x] [c=%d] [u=%d]"
            , UInt(i+2,4), tag_table(UInt(i+2)) & UInt(0xffff), counter_table(UInt(i+2)), ubit_table(UInt(i+2)))
         printf("(%d) [tag=0x%x] [c=%d] [u=%d]\n"
            , UInt(i+3,4), tag_table(UInt(i+3)) & UInt(0xffff), counter_table(UInt(i+3)), ubit_table(UInt(i+3)))
      }
      printf("\n")
