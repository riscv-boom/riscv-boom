...
import boom.exu.{BrResolutionInfo, ExeUnitResp, FuncUnitResp, FuncUnitReq}
...
class QueueForFuncUnitReq(entries: Int, data_width: Int)
   (implicit p: freechips.rocketchip.config.Parameters)
   extends boom.common.BoomModule()(p)
   with boom.common.HasBoomCoreParameters
{
   val io = IO(new Bundle
   {
      val enq     = Decoupled(new FuncUnitReq(data_width)).flip
      val deq     = Decoupled(new FuncUnitReq(data_width))
   ...
   private val ram     = Mem(entries, new FuncUnitReq(data_width))
   private val valids  = Reg(init = Vec.fill(entries) {Bool(false)})
   private val brmasks = Reg(Vec(entries, UInt(width = MAX_BR_COUNT)))
